...
export async function runCommand(hs, platform, architecture, fn, args) {
    ...
    const errorFile = script.pathToCharVec(path.join(hs.dir, "error.txt"));
    const rmcArg = `setenv('MW_ORIG_WORKING_FOLDER',cd('${script.pathToCharVec(hs.dir)}')); try, run('${hs.command}'), catch e, msg=e.message; if contains(extractBefore(msg,newline),'File:'), msg=extractAfter(msg,newline); end, fid=fopen('${errorFile}','w'); fprintf(fid,'%s',msg); fclose(fid); throwAsCaller(MException(e.identifier,'%s',msg)); end`;
    let execArgs = [rmcArg];
    if (args) {
